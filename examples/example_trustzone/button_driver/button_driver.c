...
void handle_output(void *session, uint16_t output_id, uint32_t param_types,
	...
	for(int i=0; i<num; i++){
		...
		if(connection->encryption == AES){ 
			...
			SpongentWrap(connection->connection_key, aad, 16, data, data_len * 8, output, tag_spongent, 0);
...
void button_pressed(void *session, /*unsigned char *data,*/ uint32_t param_types, TEE_Param params[4]){
